...
const recipeCardButtonAppearance = (selectedRecipe) => {
  if (currentUser.recipesToCook.map(savedRecipe => savedRecipe.id).includes(selectedRecipe.id)) {
    return `
    <p>This recipe is already in your cook list
    <button id="${selectedRecipe.id}" class="lets-cook-button">Complete This Recipe</button>`
  } else {
    return `<button id="${selectedRecipe.id}" class="lets-cook-button">Lets Cook!</button>`
  }
}
